...
@view_config(route_name='yandexmoney.cl.docc', renderer='netprofile_yandexmoney:templates/client_cc.mak')
def cc_payment(request):
	...
	if 'submit' in request.POST:
		...
		request.run_hook('yandexmoney.cl.tpldef.docc', tpldef, request)
		request.run_hook('access.cl.tpldef', tpldef, request)
		return tpldef
	...
	if 'pay' in request.POST:
		...
		if len(errors) == 0:
			...
			if yandexmoneymethod == 'classic':
				...
				yandexmoneyrestsdk.configure({
					"mode": "sandbox", # sandbox or live
					"client_id": "",
					"client_secret": "" 
				})

				payment = yandexmoneyrestsdk.Payment({
					"intent": "sale",
					"payer": {

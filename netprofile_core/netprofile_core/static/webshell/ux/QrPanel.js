...
//  some additions (c) 2012 Volker Kinkelin
//
// URL: http://www.d-project.com/
//
// Licensed under the MIT license:
//	http://www.opensource.org/licenses/mit-license.php
//	additions by Volker Kinkelin are also under MIT license
//
...
var qrcode = function () {
	...
	var qrcode = function (typeNumber, errorCorrectLevel) {
		...
		//---------------------------------------------------------------------
		// ==== additions 05.03.2012 Volker Kinkelin start ====
		//---------------------------------------------------------------------

		_this.getDOMSubTree = function (args) {

			args = args || {};

			// if code is provided, prepare this instance first ('lazy self-instanciation')
			if (args.code) {

				this.addData(args.code);
				this.make();

			}

			// get resolution (always quadratic!)
			var moduleCount = this.getModuleCount();

			// this node will be returned by the method
			var treeRoot; // is the root node for the whole QRcode div subtree

			// root node specified as a string or a DOM node or not at all?
			switch (typeof args.nodeParent) {

			case 'string':
				treeRoot = document.getElementById(args.nodeParent);
				break;

			case 'object':
				treeRoot = args.nodeParent;
				break;

			default:

				// no node specified -> create root node yourself
				// the existing DOM tree is not modified in this case (!)
				// the node must be inserted by the caller later
			...
			// rule out placeholder ("'...wait...' etc) text
			treeRoot.innerHTML = '';

			// append DIV's attributes (style, ID, title etc) if specified
			if (args.divContainerAttributes) {
			...
			// prepare template divs nodes to be cloned for black & white blocks
			var dotNodeBlack = document.createElement('div');
			var nodeAttributeBlack = document.createAttribute('class');
			nodeAttributeBlack.nodeValue = args.cssClassBlack = args.cssClassBlack || 'QB';
			dotNodeBlack.setAttributeNode(nodeAttributeBlack);

			var dotNodeWhite = document.createElement('div');
			var nodeAttributeWhite = document.createAttribute('class');
			nodeAttributeWhite.nodeValue = args.cssClassWhite = args.cssClassWhite || 'QW';
			dotNodeWhite.setAttributeNode(nodeAttributeWhite);

			// prepare row end (line-break) div node
			var dotNodeRowEnd = document.createElement('div');
			var nodeAttributeRowEnd = document.createAttribute('class');
			nodeAttributeRowEnd.nodeValue = args.cssClassRowEnd = args.cssClassRowEnd || 'QRE';
			dotNodeRowEnd.setAttributeNode(nodeAttributeRowEnd);
			...
			for (var row = 0; row < moduleCount; row++) {
				...
				for (var column = 0; column < moduleCount; column++) {
					...
					// not using _this.isDark() here for performance reasons
					if (_modules[row][column])
						treeRoot.appendChild(dotNodeBlack.cloneNode(false));
					else
						treeRoot.appendChild(dotNodeWhite.cloneNode(false));

				}

				treeRoot.appendChild(dotNodeRowEnd.cloneNode(false));
		...
		_this.drawOnCanvas = function (args) {
			...
			// if code is provided, prepare instance first ('lazy self-instanciation')
			if (args.code) {
			...
			// make sure div for canvas is given
			if (!args.divCanvas)
				return -2; // = no canvas div specified
			...
			case 'string':
				domCanvas = document.getElementById(args.divCanvas);
				break;

			case 'object':
				domCanvas = args.divCanvas;
				break;

			}

			// make sure canvas exists
			if (!domCanvas || !domCanvas.getContext)
				return -3;

			// if specified, add/update CANVAS' attributes (style, ID, title etc)
			if (args.divContainerAttributes) {
			...
			// get number of dots (black squares) in code
			var moduleCount = this.getModuleCount();
			var dotSize; // = dot size in pixels

			// adapt canvas size to dot witdh specified
			if (typeof args.dotSize == 'number') {

				dotSize = args.dotSize;

				domCanvas.setAttribute('width', moduleCount * args.dotSize);
				domCanvas.setAttribute('height', moduleCount * args.dotSize);

			} else // adapt dot width to existing canvans element size
				dotSize = Math.floor(domCanvas.width / moduleCount);

			// get drawing context
			var canvasContext = domCanvas.getContext('2d');

			// define dots fillstyle, usually solid black. background is transparent
			canvasContext.fillStyle = args.dotFillStyle = args.dotFillStyle || 'rgb(0,0,0)';
			...
			for (var row = 0; row < moduleCount; row++) {
				...
				for (var column = 0; column < moduleCount; column++) {
					...
					// not using _this.isDark() here for performance reasons
					if (_modules[row][column])
						canvasContext.fillRect(column * dotSize, row * dotSize, dotSize, dotSize);
		...
		//---------------------------------------------------------------------
		// ==== additions 05.03.2012 Volker Kinkelin end ====
		//---------------------------------------------------------------------
...
Ext.define('Ext.ux.QrPanel', {
	...
	initComponent : function () {
		...
		Ext.applyIf(me, {
			...
			textToEncode : 'qrext' // ...should be overwritten by caller
		...
		case 'canvas':
			...
			objQRCode.drawOnCanvas({
				...
				divContainerAttributes : { // container DIV's HTML attributes, optional
					...
					style : 'width:100%; height:100%; background-color:' + me.qrForegroundColor
